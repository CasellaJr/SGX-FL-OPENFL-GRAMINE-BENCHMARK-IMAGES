...
def get_writer():
    ...
    global writer
    if not writer:
        writer = SummaryWriter('./logs/cnn_svhn', flush_secs=5)
...
def load_svhn_shard(shard_num, collaborator_count,
    ...
    logger.info(f'SVHN > X_train Shape : {X_train.shape}')
    logger.info(f'SVHN > y_train Shape : {y_train.shape}')
    logger.info(f'SVHN > Train Samples : {X_train.shape[0]}')
    logger.info(f'SVHN > Valid Samples : {X_valid.shape[0]}')
